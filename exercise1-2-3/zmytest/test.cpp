...
void personalLasdTest() {
    ...
    checked = lst.Remove(5);
    corretto = (checked == 0) ? "Correct!" : "Error!";
    std::cout << "Remove 5 (List[3] {1, 2, 3}): " << checked << " - " << corretto << std::endl;

    checked = lst.Remove(2);
    corretto = (checked == 1) ? "Correct!" : "Error!";
    std::cout << "Remove 2 (List[3] {1, 2, 3}): " << checked << " - " << corretto << std::endl;
    ...
    lasd::List<int> lstCopy;
    lstCopy.operator=(lst);
    ...
    //HashTable
    std::cout << std::endl << std::endl << "~~~~~ HASHTABLE TEST ~~~~~" << std::endl;

    //Close Address
    std::cout << std::endl << "~~~ Close Address ~~~" << std::endl << std::endl;

    lasd::HashTableClsAdr<int> htc(256);

    numero = htc.Size();
    corretto = (numero == 0) ? "Correct!" : "Error!";
    std::cout << "Size (HashTableClsAdr[256]): " << numero << " - " << corretto << std::endl;

    for(unsigned long i = 0; i < 127; i+=7){
      checked = htc.Insert(i*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Insertion of pseudocasual number " << i*5 << " - " << corretto << std::endl;
    }

    for(unsigned long i = 0, j = 0; i < 19; i++, j += 7){
      checked = htc.Exists(j*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Exists test of the value " << j*5 << " - " << corretto << std::endl;
    }    

    for(unsigned long i = 0, j = 0; i < 3; i++, j += 7){
      checked = htc.Remove(j*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Remove have to success: " << j*5 << " - " << corretto << std::endl;
    }

    for(unsigned long i = 0, j = 0; i < 15; i++, j++){
      checked = htc.Exists(j*7);
      corretto = (checked == 0) ? "Correct!" : "Error!";
      std::cout << "Not Exists test of the value " << j*7 << " - " << corretto << std::endl;
    }

    htc.Resize(500);
    numero = htc.Size();
    corretto = (numero == 16) ? "Correct!" : "Error!";
    std::cout << "Size (HashTableClsAdr[16]): " << numero << " - " << corretto << std::endl;
    ...
    for(unsigned long i = 0, j = 0; i < 3; i++, j += 7){
      checked = htc.Remove(j*5);
      corretto = (checked == 0) ? "Correct!" : "Error!";
      std::cout << "Remove have to fail: " << j*5 << " - " << corretto << std::endl;
    }

    lasd::HashTableClsAdr<int> htc2 = htc;
    checked = htc.operator==(htc2);
    corretto = (checked == 1) ? "Correct!" : "Error!";
    std::cout << "Comparison two equal table after copy assignment: " << checked << " - " << corretto << std::endl;


    htc.Clear();
    numero = htc.Size();
    corretto = (numero == 0) ? "Correct!" : "Error!";
    std::cout << "Size (HashTableClsAdr[0]): " << numero << " - " << corretto << std::endl;

    checked = htc.Empty();
    corretto = (checked == 1) ? "Correct!" : "Error!";
    std::cout << "Empty (HashTableClsAdr[0]: " << checked << " - " << corretto << std::endl;


    //Open Address
    std::cout << std::endl << "~~~ Open Address ~~~" << std::endl << std::endl;

    lasd::HashTableOpnAdr<int> hto;
    ...
    for(unsigned long i = 0; i < 127; i+=7){
      checked = hto.Insert(i*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Insertion of pseudocasual number " << i*5 << " - " << corretto << std::endl;
    }

    for(unsigned long i = 0, j = 0; i < 19; i++, j += 7){
      checked = hto.Exists(j*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Exists test of the value " << j*5 << " - " << corretto << std::endl;
    }    

    for(unsigned long i = 0, j = 0; i < 3; i++, j += 7){
      checked = hto.Remove(j*5);
      corretto = (checked == 1) ? "Correct!" : "Error!";
      std::cout << "Remove have to success: " << j*5 << " - " << corretto << std::endl;
    }

    for(unsigned long i = 0, j = 0; i < 15; i++, j++){
      checked = hto.Exists(j*7);
      corretto = (checked == 0) ? "Correct!" : "Error!";
      std::cout << "Not Exists test of the value " << j*7 << " - " << corretto << std::endl;
    }
    ...
    lasd::HashTableOpnAdr<int> hto2 = hto;
    checked = hto.operator==(hto2);
    corretto = (checked == 1) ? "Correct!" : "Error!";
    std::cout << "Comparison two equal table after copy assignment: " << checked << " - " << corretto << std::endl;
