...
void BinaryTree<Data>::Fold(FoldFunctor funct, void* acc) const{
    if(!Container::Empty())
        PreOrderFold(funct, acc);
}

template<typename Data>
void BinaryTree<Data>::PreOrderFold(FoldFunctor funct, void* acc) const{
    if(!Container::Empty())
...
template<typename Data>
void BinaryTree<Data>::PostOrderFold(FoldFunctor funct, void* acc) const{
    if(!Container::Empty())
        PostOrderFold(funct, acc, Root());
}

template<typename Data>
void BinaryTree<Data>::PostOrderFold(FoldFunctor funct, void* acc, Node & current) const{
    if(current.HasLeftChild())
        PostOrderFold(funct, acc, current.LeftChild());
    if(current.HasRightChild())
        PostOrderFold(funct, acc, current.RightChild());
    funct(current.Element(), acc);
}

template<typename Data>
void BinaryTree<Data>::InOrderFold(FoldFunctor funct, void* acc) const{
    if(!Container::Empty())
        InOrderFold(funct, acc, Root());
}

template<typename Data>
void BinaryTree<Data>::InOrderFold(FoldFunctor funct, void* acc, Node & current) const{
    if(current.HasLeftChild())
        InOrderFold(funct, acc, current.LeftChild());
    funct(current.Element(), acc);
    if(current.HasRightChild())
        InOrderFold(funct, acc, current.RightChild());
}

template<typename Data>
void BinaryTree<Data>::BreadthFold(FoldFunctor funct, void* acc) const{
    if(!Container::Empty())
        BreadthFold(funct, acc, Root());
}

template<typename Data>
void BinaryTree<Data>::BreadthFold(FoldFunctor funct, void* acc, Node & current) const{
    QueueLst<Node*> queue;
    Node * tmp = &Root();
    queue.Enqueue(tmp);
    while (!queue.Empty())
